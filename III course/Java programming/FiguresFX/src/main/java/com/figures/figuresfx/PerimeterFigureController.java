...
public class PerimeterFigureController {
    @FXML
    public ComboBox<String> comboBoxp;
    ...
    public void initialize(){
        for(int i = 0; i < Storage.getListFigures().size(); i++) {
            ...
            switch (figure) {
                case "Окружность":
                    comboBoxp.getItems().add("Окружность: " + "Circle" + Storage.getListFigures().get(i).toString());
                    break;
                case "Отрезок":
                    comboBoxp.getItems().add("Отрезок: " + "Segment" + Storage.getListFigures().get(i).toString());
                    break;
                ...
                case "Прямоугольник": // Если фигура - прямоугольник
                    ...
                    if (figure == "Ломанная") {
                        ...
                        comboBoxp.getItems().add("Ломанная: " + "Polyline" + Storage.getListFigures().get(i).toString());
                    }
                    if (figure == "Многоугольник") {

                        comboBoxp.getItems().add("Многоугольник: " + "NGon" + Storage.getListFigures().get(i).toString());
                    }
                    if (figure == "Четырёхугольник") {

                        comboBoxp.getItems().add("Четырёхугольник: " + "QGon" + Storage.getListFigures().get(i).toString());
                    }
                    if (figure == "Треугольник") {

                        comboBoxp.getItems().add("Треугольник: " + "TGon" + Storage.getListFigures().get(i).toString());
                    }
                    if (figure == "Трапеция") {

                        comboBoxp.getItems().add("Трапеция: " + "Trapeze" + Storage.getListFigures().get(i).toString());
                    }
                    if (figure == "Прямоугольник") {

                        comboBoxp.getItems().add("Прямоугольник: " + "Rectangle" + Storage.getListFigures().get(i).toString());
                    }
                    break;
            }
            String type;
    ...
    public void buttons_Click(ActionEvent actionEvent) {
        try
        {
            ...
            double perimetr = Storage.listFigures.get(comboBoxp.getSelectionModel().getSelectedIndex()).length();

            mainController.label61.setText("Периметр: ");
            mainController.textBox41.setText(String.valueOf(perimetr));
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setTitle("Успех");
            alert.setHeaderText(null);
            alert.setContentText("Периметр подсчитан");
            alert.showAndWait();
